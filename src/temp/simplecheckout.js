(function($) {
    window.Simplecheckout = function(params) {
        ...
        this.clickOnConfirmButton = function() {
            ...
            $('#button-confirm').addClass('clickRed');
        ...
        this.proceed = function() {
            ...
            if (self.params.displayProceedText && !self.isPaymentFormVisible()) {
                $(self.params.mainContainer).find(self.selectors.buttonCreate).addClass('clickRed');
                self.overlayAll();
        ...
        this.nextStep = function($target) {
            ...
            var self = this;
            if (!self.validate()) {
                $(self.params.mainContainer).find(self.selectors.buttonCreate).removeClass('clickRed');
                return;
            }
            /*if (!$('#agree').is(":checked")) {
                $(self.params.mainContainer).find(self.selectors.agreementWarning).show();
                $(self.params.mainContainer).find(self.selectors.buttonNext).addClass('grey');
                return;
            }
            else {
                $(self.params.mainContainer).find(self.selectors.agreementWarning).hide();
            }*/
        ...
        this.displayCurrentStep = function() {
            ...
            var initButtons = function() {
                if (self.stepsCount > 1) {
                    console.log('self.stepsCount='+self.stepsCount+' self.currentStep'+self.currentStep);
                    if (self.currentStep == 1) {
                        $mainContainer.find(self.selectors.buttonPrev).hide();
                        $mainContainer.find(self.selectors.buttonBack).show();
                        $mainContainer.find(self.selectors.buttonNext).html(button_submit_order);
                        $mainContainer.find(self.selectors.buttonBack).html(button_continue_shopping);
                        if (!$('#agree').is(":checked")) {
                            $('#anti_button_next').show();
                            $('#simplecheckout_button_next').hide();

                        }
                        else {
                            $('#anti_button_next').hide();
                        }
                        //  $('#paymentNotice').show();
                    ...
                    if (self.currentStep < self.stepsCount && $('#agree').is(":checked")) {
                    ...
                    $mainContainer.find(self.selectors.agreementCheckBox).parent().hide();
                    $mainContainer.find(self.selectors.agreementWarning).hide();

                    if (self.currentStep == 1) {
                        $mainContainer.find(self.selectors.agreementCheckBox).parent().show();
                        // $mainContainer.find(self.selectors.agreementWarning).show();
                    }
                }
                if (self.currentStep == 3 && self.stepsCount > 3) {
                    $mainContainer.find(self.selectors.buttonNext).hide();
                    // $('#registr-but').append($mainContainer.find(self.selectors.buttonNext));
                    //login.open();
                }
                if (self.currentStep == self.stepsCount) {
                    $mainContainer.find(self.selectors.buttonNext).hide();
            ...
            var initStepsMenu = function() {
                console.log("Текущий шаг оформления заказа: " + self.currentStep);
                $('body').find(self.selectors.buttons).removeClass('step1 step2 step3 step4');
                $('body').find(self.selectors.buttons).addClass('step'+self.currentStep);
                ...
                /* $mainContainer.find(self.selectors.stepsMenu + " " + self.selectors.stepsMenuItem).removeClass(self.classes.stepsMenuCompleted).removeClass(self.classes.stepsMenuCurrent);
                 $mainContainer.find(self.selectors.stepsMenu + " " + self.selectors.stepsMenuDelimiter + " img").attr("src", self.params.additionalPath + self.resources.next);
 */
                if (self.currentStep==1) {/*$('.finishLine').hide();*/ $('.priceAndDelivery').hide(); }
                else {
                    if (self.stepsCount == 3){
                        /* if (self.currentStep-1 == 2)
                             $('.finishLine').removeClass().addClass('finishLine half item2');
                         else*/
                        $('.finishLine').removeClass().addClass('finishLine half item'+(self.currentStep-1));

                    }
                    else
                        $('.finishLine').removeClass().addClass('finishLine item'+(self.currentStep-1));
                }
                /* $mainContainer.find(self.selectors.stepsMenu + " " + self.selectors.stepsMenuItem + "[data-step=" + self.currentStep + "]").addClass(self.classes.stepsMenuCurrent);*/
            };
        ...
        this.scroll = function() {
            ...
            if (self.params.scrollToError) {
                console.log('START2');
                $($mainContainer.find("[data-valid=false]:visible")).each(function() {
                    $parent_nice = $(this).parent('.niceInput');
                    //if (!$parent_nice.hasClass('error')) {
                    $parent_nice.addClass('error');
                    console.log('parent nice errro');
                    // }
                    var offset = $(this).offset();
                    if (offset.top < top) {
                        console.log('scrollToError1');
                        top = offset.top;
                    }
                    if (offset.bottom > bottom) {
                        bottom = offset.bottom;
                        console.log('scrollToError2');
                    }
                });
                ...
                $($mainContainer.find(".simplecheckout-warning-block:visible")).each(function() {
                    ...
                    if (offset.top < top) {
                        top = offset.top;
                        console.log('simplecheckout-warning-');
                    }
                    if (offset.bottom > bottom) {
                        bottom = offset.bottom;
                        console.log('simplecheckout-warning-2');
                    }
                });
                ...
                if (top < 10000 && isOutsideOfVisibleArea(top)) {
                    ...
                    console.log('isOutsideOfVisibleArea1');
                    error = true;
                } else if (bottom && isOutsideOfVisibleArea(bottom)) {
                    $("html, body").animate({
                        scrollTop: bottom
                    }, "slow");
                    console.log('isOutsideOfVisibleAre2');
                    error = true;
                }
            }
            ...
            if (self.params.scrollToPaymentForm && !error) {
                if (self.isPaymentFormVisible()) {
                    console.log('scrollToPaymentForm');
            ...
            console.log('self.saveStepNumber = self.currentStep');
        ...
        this.createOrder = function() {
            var self = this;
            $('#customer_last_click').val(1);
            if (!self.validate()) {
                $('#customer_last_click').val('');
                $(self.params.mainContainer).find(self.selectors.buttonCreate).removeClass('clickRed');
                return;
            }
    ...
    function SimplecheckoutCart(container, route) {
        ...
        this.changeProductQuantity = function($target) {
            ...
            if (typeof $target[0] !== "undefined" && typeof $target[0].tagName !== "undefined" && $target[0].tagName !== "INPUT") {
                $target = $target.parents("div").find("input");
            }
    ...
    function SimplecheckoutLogin(container, route) {
        ...
        this.initPopupLayer = function() {
            ...
            if (!$("#simple_login_layer").length || position == "fixed" || position == "relative" || position == "absolute") {
                ...
                $("#simple_login").remove();
                $(self.params.mainContainer).append("<div id='simple_login_layer' class='popupAuth__back'><div class='simplecheckout_overlay'></div></div><div id='simple_login' class='popupAuth' ><div id='temp_popup_container'></div></div>");
                $("#simple_login_layer").on("click", function() {
                    self.close();
                });

            }
            $(".popupAuth__close").on("click", function(e) {
                e.preventDefault();
                self.close();
            });
            $("#simple_login_layer")
        ...
        this.resizePopup = function() {
            $("#simple_login").show();
            $("#simple_login").css("height", $(this.currentContainer).outerHeight() + 50);
            // $("#simple_login").css("top", $(window).height() / 2 - ($("#simple_login").outerHeight() ? $("#simple_login").outerHeight() : $("#simple_login").height()) / 2);
            //f $("#simple_login").css("left", $(window).width() / 2 - ($("#simple_login").outerWidth() ? $("#simple_login").outerWidth() : $("#simple_login").width()) / 2);
        };
        ...
        this.open = function() {
            ...
            //$('#authorizeBlock > .down ').prepend($('#simplecheckout_login'));
